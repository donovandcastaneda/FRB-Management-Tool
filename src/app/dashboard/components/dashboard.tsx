...
import { RecentTransactions } from "./recent-transcations";
...
const Dashboard: React.FC = () => {
  ...
  const [pendingBalance, setPendingBalance] = useState<Balance[]>([]);

  useEffect(() => {
    const fetchBalance = async () => {
      try {
        const response = await fetch("/api/stripe/get-balances");
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              ...
              {availableBalance.length > 0 ? (
                availableBalance.map((balance, index) => (
                  <li key={index}>{formatCurrency(balance.amount)}</li>
                ))
              ) : (
                <p>Not Available</p>
              )}
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              ...
              {pendingBalance.length > 0 ? (
                pendingBalance.map((balance, index) => (
                  <ul key={index}>{formatCurrency(balance.amount)}</ul>
                ))
              ) : (
                <p>Not Available</p>
              )}
